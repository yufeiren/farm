/* modified from http://brokestream.com/procstat.c
...
num readcpu() {
  num cputime[10];
  inputstat = fopen("/proc/stat", "r");
  if(!inputstat) {
    perror("fopen");
    exit(1);
  }
  
  fscanf(inputstat, "cpu  %lld %lld %lld %lld %lld %lld %lld %lld \n",
    &cputime[0], &cputime[1], &cputime[2], &cputime[3],
    &cputime[4], &cputime[5], &cputime[6], &cputime[7]);
  fclose(inputstat);
  
  return cputime[0] + cputime[1] + cputime[2] + cputime[3] +
    cputime[4] + cputime[5] + cputime[6] + cputime[7];
}
...
char datfile[32];
memset(datfile, '\0', 32);
snprintf(datfile, 32, "./cpuusage_%s.dat", argv[1]);
cupusage = fopen(datfile, "a");
if (!cupusage) {
      perror("fopen");
      return 1;
    }
