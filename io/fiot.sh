...
# local
	...
	rm -rf $LocalDiskFileOut
	sync; echo 3 > /proc/sys/vm/drop_caches
	$Fiotool -w -s -f $LocalDiskFileOut -m $bufsiz -t $FileoutSize -i 2 > $Logdir/WR_Stream_Local_M$bufsiz.txt 2>&1
	sleep 5;
	rm -rf $LocalDiskFileOut
	sync; echo 3 > /proc/sys/vm/drop_caches
...
# lustre
	...
	sync; echo 3 > /proc/sys/vm/drop_caches
	$Fiotool -w -f $LustreDiskFileOut -m $bufsiz -t $FileoutSize -i 2 > $Logdir/WR_Lustre_M$bufsiz.txt 2>&1
	sleep 5;
	rm -rf $LustreDiskFileOut
	sync; echo 3 > /proc/sys/vm/drop_caches
	$Fiotool -w -s -f $LustreDiskFileOut -m $bufsiz -t $FileoutSize -i 2 > $Logdir/WR_Stream_Lustre_M$bufsiz.txt 2>&1
	sleep 5;
	rm -rf $LustreDiskFileOut
	sync; echo 3 > /proc/sys/vm/drop_caches
	$Fiotool -w -o -f $LustreDiskFileOut -m $bufsiz -t $FileoutSize -i 2 > $Logdir/WR_O_Direct_Lustre_M$bufsiz.txt 2>&1
done
